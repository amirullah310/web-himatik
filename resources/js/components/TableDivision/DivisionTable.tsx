...
export function DivisionTable({ data, onEdit, periods = [], selectedPeriod = '' }) {
  ...
  function handlePeriodFilter(value: string) {
    window.location.href =
      value === '__all__' ? '/divisions' : `/divisions?period_id=${value}`;
  }
      ...
      cell: ({ row }: any) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            >
              <List className="mr-2 h-4 w-4" /> Lihat Proker Divisi
            </DropdownMenuItem>
  ...
  return (
    <>
      ...
      {/* Responsive Table */}
      <div className="w-full overflow-x-auto rounded-lg border border-muted/40 dark:border-zinc-800">
        <Table className="min-w-full text-sm">
          <TableHeader className="sticky top-0 z-10 bg-muted/60 dark:bg-zinc-900/80 backdrop-blur-md">
          ...
          <TableBody>
            {table.getRowModel().rows.length === 0 ? (
              ...
              table.getRowModel().rows.map((row, idx) => (
                <TableRow
                  key={row.id}
                  className={`transition hover:bg-purple-50/60 dark:hover:bg-zinc-800/60 ${
                ...
                >
                  {row.getVisibleCells().map((cell) => (
                    <TableCell
                      key={cell.id}
                      className="px-4 py-3 text-sm text-muted-foreground border-l border-r border-transparent dark:border-zinc-800 whitespace-nowrap sm:whitespace-normal"
                    >
